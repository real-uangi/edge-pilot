...
func TestLoadAgentRuntimeConfigEnvOverridesBuildVersion(t *testing.T) {
	t.Setenv("AGENT_VERSION", "manual-override")
	...
	cfg := LoadAgentRuntimeConfig()
	if cfg.AgentVersion != "manual-override" {
		t.Fatalf("expected env override, got %q", cfg.AgentVersion)
	}
}
